            ...
            mPresenter.getHisSport(hisSportEntity.getStartTime(), hisSportEntity.getEndTime(), kithEntity.getKithCustCode(), new OnTrackListener() {
                ...
                public void onHistoryTrackCallback(HistoryTrackResponse historyTrackResponse) {
                    ...
                    if ( mPresenter.dateToStamp(trackPoint.getCreateTime())/1000>=list.get(0).getStartTime()&& mPresenter.dateToStamp(trackPoint.getCreateTime())/1000<list.get(0).getEndTime()) {
                        ...
                        //}
                        if (list.size()==1){
                            LogUtils.d("起点：lng:"+trackPoint.getLocation().longitude+" lat:"+trackPoint.getLocation().latitude);
                        }
                    }else {
                       ...
                       // if (list.get(list.size()-1).getStartLatLng() == null ){
                            list.get(list.size()-1).setStartLatLng(trackPoint.getLocation());
                        //}
                        //if (list.get(list.size()-1).getEndLatLng() == null ){
                ...
                .map(hisSportEntities -> {
                    ...
                    for (int i=0;i< hisSportEntities.size();i++) {
                        ...
                        if (hisSportEntity.getStartLatLng()!=null){
                            ...
                            LogUtils.d("i:"+i+"  dd:"+distance);
                            if (list.size()==1){
                                LogUtils.d("起点：lng:2 "+hisSportEntity.getStartLatLng().longitude+" lat:"+hisSportEntity.getStartLatLng().latitude);
                                LogUtils.d("起点：lng:3 "+reverseGeoCodeResult.getLocation().longitude+" lat:"+reverseGeoCodeResult.getLocation().latitude);
                            }
                            if (distance<500){
